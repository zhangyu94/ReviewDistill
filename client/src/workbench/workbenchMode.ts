...
export function entryMode(routeName: string | symbol | undefined | null): EntryMode {
  if (routeName === 'disappeared') { return 'disappeared' }
  if (routeName === 'taxonomy' || routeName === 'issue') { return 'observations' }
  return 'uncoded'
}

export function inspectorKind(routeName: string | symbol | undefined | null): InspectorKind {
  if (routeName === 'taxonomy' || routeName === 'issue') { return 'issue' }
  return 'comment'
}
...
export function dismissTypeHref(selector: SelectorId): string {
  if (selector === 'disappeared') { return '/inbox/disappeared' }
  return '/'
}
