...
from reviewdistill.labeling.coder import build_prompt, label_unlabeled_comments, parse_model_output
from reviewdistill.db.models import LABEL_ACTIVE, Assignment, Label, LabelExample, ProofreadingComment
...
def test_label_unlabeled_comments_writes_accepted_assignment(db, tmp_path):
    repo = tmp_path / "paper"
    repo.mkdir()
    ...
    summary = label_unlabeled_comments(provider=provider)
...
def test_label_unlabeled_comments_stores_manuscript_context_as_example(db, tmp_path):
    repo = tmp_path / "paper"
    repo.mkdir()
    init_project(name="paper-01", commands=["myremark"], cwd=repo)
...
def test_label_new_name_mints_one_label_for_the_batch(db, tmp_path):
    repo = tmp_path / "paper"
    repo.mkdir()
    init_project(name="paper-01", commands=["myremark"], cwd=repo)
    ...
    with get_session() as session:
        ...
        accepted = session.find(Assignment, status="accepted")
        assert len(accepted) == 2
        assert {row.label_id for row in accepted} == {labels[0].id}


def test_label_skips_new_recommendation_without_label_name(db, tmp_path):
    ...
    with get_session() as session:
        assert session.find(Assignment) == []
    assert len(unlabeled_for_ai(provider_name="openai")) == 1


def test_unlabeled_includes_incomplete_new_proposal(db, tmp_path):
    from reviewdistill.labeling.coder import unlabeled_for_ai
    ...
    with get_session() as session:
        ...
        session.commit()
    assert [row.id for row in unlabeled_for_ai(provider_name="openai")] == [comment.id]


def test_unlabeled_includes_accepted_label_on_inactive_type(db, tmp_path):
    from reviewdistill.labeling.coder import unlabeled_for_ai
    ...
    with get_session() as session:
        ...
        session.commit()
        comment_id = comment.id
    assert [row.id for row in unlabeled_for_ai(provider_name="openai")] == [comment_id]


def test_label_skips_already_resolved_comments(db, tmp_path):
    repo = tmp_path / "paper"
    repo.mkdir()
    init_project(name="paper-01", commands=["myremark"], cwd=repo)
    (repo / "main.tex").write_text("\\myremark{Too strong.}\n")
    extract_project(repo)
    provider = MockLLMProvider()
    first = label_unlabeled_comments(provider=provider)
    second = label_unlabeled_comments(provider=provider)
    assert first.assigned == 1
    assert second.assigned == 0


def test_label_replaces_placeholder_mock_proposal(db, tmp_path):
    repo = tmp_path / "paper"
    repo.mkdir()
    init_project(name="paper-01", commands=["myremark"], cwd=repo)
    ...
    summary = label_unlabeled_comments(provider=_Stub())
    assert summary.assigned == 1
    with get_session() as session:
        rows = session.find(Assignment, comment_id=comment_id)
        assert session.get(Assignment, "mock-proposed") is None
        assert len(rows) == 1
...
def test_unknown_label_id_does_not_fall_back_to_top_candidate(db, tmp_path):
    ...
    label_unlabeled_comments(provider=provider)
    with get_session() as session:
        coding = session.first(Assignment)
...
def test_label_continues_after_one_unparseable_response(db, tmp_path):
    repo = tmp_path / "paper"
    repo.mkdir()
    init_project(name="paper-01", commands=["myremark"], cwd=repo)
...
def test_label_unlabeled_comments_ranks_all_comments_without_reloading_store(db, tmp_path, monkeypatch):
    repo = tmp_path / "paper"
    repo.mkdir()
    init_project(name="paper-01", commands=["myremark"], cwd=repo)
    (repo / "main.tex").write_text("\\myremark{First comment.}\n\\myremark{Second comment.}\n")
    ...
    from reviewdistill.labeling import coder as coder_mod
    ...
    summary = label_unlabeled_comments(provider=MockLLMProvider())
    assert summary.assigned == 2
...
def test_label_aborts_provider_http_error(db, tmp_path):
    repo = tmp_path / "paper"
    repo.mkdir()
    init_project(name="paper-01", commands=["myremark"], cwd=repo)
    ...
    with pytest.raises(httpx.HTTPError):
        label_unlabeled_comments(provider=_Down())


def test_label_unlabeled_comments_commits_once(db, tmp_path, monkeypatch):
    repo = tmp_path / "paper"
    repo.mkdir()
    init_project(name="paper-01", commands=["myremark"], cwd=repo)
    (repo / "main.tex").write_text("\\myremark{First comment.}\n\\myremark{Second comment.}\n")
