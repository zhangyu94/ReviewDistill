...
- **Comment inspector** — wording, manuscript context, type, quality (Verify / Drop), location. When the `.tex` file is still on this computer, **File** is a control that selects it in the file manager (same idea as Settings **Open folder**: not a `file://` link, and not the default editor). If the checkout is gone, File stays plain text. Failures show in the Comments error line. Unlabeled **Assign type** is two rows: a stored proposal (or **No AI suggestion**) then a type menu of **leaves** (a leftover parent label still shows as the current value). The menu shows the current type (or **Choose a type…** if unlabeled). Picking a type assigns it; there is no Change button. **Accept** applies the AI suggestion. New types are titled `New: {name}`; a new proposal with no name is **No AI suggestion** and stays in the Label with AI queue. There is no Reject: not accepting a suggestion leaves the comment unlabeled.
