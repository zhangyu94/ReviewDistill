...
import { fetchHistory, redoHistory, undoHistory, type HistoryEvent } from '../api/client.ts'
...
const selectedId = computed(() => {
  ...
  if (id && events.value.some(event => event.id === id))
    return id
...
<template>
  <div class="flex h-full min-h-0 flex-col bg-[var(--ch-color-background)]">
    <div class="flex shrink-0 items-center gap-1.5 border-b border-[var(--ch-color-border)] px-2 py-1.5">
      ...
      <p v-if="error" class="ch-muted-text truncate">{{ error }}</p>
    </div>
    <div class="flex min-h-0 flex-1">
      <div class="w-80 shrink-0 overflow-auto border-r border-[var(--ch-color-border)] text-xs">
        ...
        >
          <div class="uppercase tracking-[0.06em]">{{ event.event_type }}</div>
          <strong :class="event.undone ? 'font-normal' : ''">{{ event.summary }}</strong>
          <div class="ch-muted-text">{{ event.created_at }}</div>
        </button>
        <p v-if="events.length === 0" class="ch-muted-text p-2">No events.</p>
      </div>
      <div class="min-h-0 min-w-0 flex-1 overflow-auto p-3 text-xs">
        <template v-if="selected">
          <p class="mb-0.5"><strong>{{ selected.summary }}</strong></p>
          <p class="ch-muted-text mb-2">{{ selected.event_type }} · {{ selected.created_at }}</p>
          <p v-if="selected.undone" class="ch-muted-text mb-2">Undone</p>
