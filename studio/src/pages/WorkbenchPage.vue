...
import { dropAction, type DragPayload, type DropTarget } from '../workbench/dropAction.ts'
import {
  activeSelector,
  allowChangeDrop,
  disappearedHref,
...
const selectedCommentId = computed(() => {
  ...
  if (id && queueItems.value.some(item => item.comment.id === id))
    return id
...
const listSelectedId = computed(() => {
  if (mode.value !== 'observations')
    return selectedCommentId.value
...
const inspectorView = computed(() => {
  if (mode.value === 'disappeared')
    return 'disappeared' as const
  if (mode.value === 'observations')
...
const suggestionTitle = computed(() => {
  ...
  if (!codingRow)
    return null
  if (codingRow.kind === 'new')
    return codingRow.proposed_issue_name || 'New issue type'
...
function codeAllTitle(): string {
  if (!inbox.value?.llm_provider)
    return 'Configure llm.provider and .reviewdistill/.env first'
  if (!inbox.value.pending_code_count)
    return 'No uncoded comments need suggestions'
...
async function loadInbox() {
  ...
  if (inbox.value.issues[0] && !changeId.value)
    changeId.value = inbox.value.issues[0].id
}
...
async function codeAll() {
  ...
  try {
    ...
    if (result.privacy_warning)
      notice.value = result.privacy_warning
    await loadInbox()
    await loadTaxonomy()
  }
  catch (err) {
...
async function onDrop(payload: DragPayload, target: DropTarget) {
  ...
  if (action.type === 'ignore' || !allowChangeDrop(mode.value, action.type))
    return
  error.value = ''
  try {
    ...
    if (action.type === 'merge') {
      ...
      if (selector.value === 'type')
        await router.push(`/taxonomy/${action.targetId}`)
      else
        await router.replace({ query: { type: action.targetId, ...(selectedCommentId.value ? { id: selectedCommentId.value } : {}) } })
      return
    }
...
function onCommentsLayout(layout: CommentsLayout) {
  ...
  if (layout === 'one' && mode.value === 'observations' && !selectedObservationId.value)
    selectedObservationId.value = issue.value?.comments[0]?.id
}
...
async function onTaxonomyChanged() {
  await loadAll()
  if (groupId.value && !typeRow(groupId.value))
    await onIssueRemoved()
}
    ...
    <div class="flex min-h-0 flex-1">
      ...
      <div class="flex w-[28rem] shrink-0 flex-col bg-[var(--ch-color-background)]">
        ...
        >
          ...
          />
          <p v-else class="ch-muted-text">Select a comment.</p>
        </EntriesPanel>
      </div>
    </div>
  </div>
