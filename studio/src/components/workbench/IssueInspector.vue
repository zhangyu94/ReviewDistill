...
async function saveType() {
  ...
  await wrap(async () => {
    ...
    if (editCategory.value !== issue.category)
      await moveIssue(issue.id, editCategory.value)
    emit('updated')
  }, () => {
    editingType.value = false
  })
...
<template>
  <div>
    <p v-if="!selectedId" class="ch-muted-text">Select a group.</p>
    <p v-else-if="missing">This issue type was not found.</p>
    <template v-else-if="issue">
      <div class="flex w-full flex-col gap-3">
        <section class="ch-panel">
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <h2 class="ch-kicker mb-0">Issue type</h2>
            <button
              v-if="!editingType"
          ...
          <dl class="grid grid-cols-[7.5rem_1fr] gap-x-2 gap-y-1 leading-4">
            <dt class="ch-muted-text">Name</dt>
            <dd v-if="!editingType" class="font-semibold text-[var(--ch-color-foreground)]">{{ issue.name }}</dd>
            <dd v-else><input v-model="editName" class="ch-input"></dd>
            <dt class="ch-muted-text">Code</dt>
            <dd v-if="!editingType" class="break-all font-[var(--ch-font-mono)]">{{ issue.code }}</dd>
            <dd v-else><input v-model="editCode" class="ch-input font-[var(--ch-font-mono)]"></dd>
            <dt class="ch-muted-text">Category</dt>
            <dd v-if="!editingType">{{ issue.category }}</dd>
            <dd v-else><input v-model="editCategory" class="ch-input" placeholder="e.g. Argumentation"></dd>
            <dt class="ch-muted-text">Accepted comments</dt>
            <dd>{{ selectedCount }}</dd>
          </dl>
        </section>
        ...
        <section class="ch-panel">
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <h2 class="ch-kicker mb-0">Definition</h2>
            <button
          ...
          <template v-if="!editingDefinition">
            <p class="ch-prose whitespace-pre-wrap text-[var(--ch-color-body)]">{{ issue.definition }}</p>
            <template v-if="issue.notes">
              <p class="ch-muted-text mt-2">Notes</p>
              <p class="ch-prose mt-0.5 whitespace-pre-wrap">{{ issue.notes }}</p>
            </template>
        ...
        <details class="ch-panel">
          <summary class="cursor-pointer font-medium">Split into two types</summary>
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div>
              <h3 class="mb-1.5 font-medium">First type</h3>
            ...
            <div>
              <h3 class="mb-1.5 font-medium">Second type</h3>
              <label class="ch-field-label">Code</label>
