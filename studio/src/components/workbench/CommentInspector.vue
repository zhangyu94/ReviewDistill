...
function onChangeId(value: unknown) {
  if (typeof value === 'string')
    emit('update:changeId', value)
}
</script>

<template>
  <div>
    <p v-if="error" class="ch-error-text mb-3">{{ error }}</p>
    <p v-if="notice" class="ch-muted-text mb-3">{{ notice }}</p>
    <template v-if="selected">
      <div class="flex max-w-3xl flex-col gap-3">
        <section class="ch-panel">
          <h2 class="ch-kicker">Comment</h2>
          <p class="ch-prose whitespace-pre-wrap">{{ selected.comment.raw_text }}</p>
        </section>

        <section class="ch-panel-muted">
          <h2 class="ch-kicker">Manuscript context</h2>
          <p class="font-[var(--ch-font-mono)] leading-5 whitespace-pre-wrap text-[var(--ch-color-body)]">{{ contextParts.prose || '—' }}</p>
          <p v-if="contextParts.extras" class="ch-muted-text mt-2 leading-4">{{ contextParts.extras }}</p>
        </section>

        <section class="ch-panel">
          <h2 class="ch-kicker">Location</h2>
          <dl class="grid grid-cols-[7.5rem_1fr] gap-x-2 gap-y-1">
            <template v-for="row in locationRows" :key="row.label">
              <dt class="ch-muted-text">{{ row.label }}</dt>
        ...
        <details class="ch-panel">
          <summary class="cursor-pointer font-medium">Record details</summary>
          <dl class="mt-2 grid grid-cols-[7.5rem_1fr] gap-x-2 gap-y-1 leading-4">
            <dt class="ch-muted-text">ID</dt><dd class="break-all">{{ selected.comment.id }}</dd>
            <dt class="ch-muted-text">Source type</dt><dd>{{ selected.comment.source_type }}</dd>
            <dt class="ch-muted-text">Status</dt><dd>{{ selected.comment.status }}</dd>
            <dt class="ch-muted-text">Git commit</dt><dd class="break-all">{{ selected.comment.git_commit || '—' }}</dd>
            <dt class="ch-muted-text">Fingerprint</dt><dd class="break-all">{{ selected.comment.fingerprint }}</dd>
            <dt class="ch-muted-text">Created</dt><dd>{{ selected.comment.created_at }}</dd>
            <template v-if="selected.comment.supersedes_id">
              <dt class="ch-muted-text">Supersedes</dt><dd class="break-all">{{ selected.comment.supersedes_id }}</dd>
            </template>
          </dl>
        </details>
        ...
        <template v-if="view === 'uncoded'">
          <section class="ch-panel">
            <h2 class="ch-kicker">Assign type</h2>
            <template v-if="selected.coding">
              ...
              <p class="font-semibold">{{ suggestionTitle }}</p>
              <p class="ch-prose mt-1 text-[var(--ch-color-body)]">{{ selected.coding.rationale }}</p>
            </template>
            ...
            <div class="mt-3 border-t border-[var(--ch-color-border)] pt-3">
              <p class="ch-muted-text mb-1.5">Apply this suggestion, or skip coding this comment</p>
              ...
              <p class="ch-muted-text mb-1.5 mt-3">Or assign an existing issue type instead</p>
              <div class="flex flex-wrap items-center gap-1.5">
        ...
        <template v-else-if="view === 'disappeared'">
          <section class="ch-panel">
            <h2 class="ch-kicker">Disappeared</h2>
            <p class="mb-3">{{ selected.guess }}</p>
            <div class="flex flex-wrap items-center gap-1.5">
              <button
                class="ch-btn ch-btn-default"
                type="button"
