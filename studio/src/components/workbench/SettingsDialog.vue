...
<template>
  ...
  >
    <div class="w-full max-w-md rounded-[4px] border border-[var(--ch-color-border)] bg-[var(--ch-color-background)] p-4 text-xs shadow-lg">
      ...
      <p v-if="notice && panel === 'assistant'" class="ch-muted-text mb-2">{{ notice }}</p>

      <template v-if="panel === 'data'">
        <p class="ch-muted-text mb-1">Comments and issue types live in this folder:</p>
        <p class="mb-1 break-all font-mono text-[11px]">{{ location?.home ?? '…' }}</p>
        <button
