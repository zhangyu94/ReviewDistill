...
export function inboxLocationRows(args: {
  ...
  if (args.heading)
    rows.push({ label: 'Heading', value: args.heading, href: null })
  if (args.gitUrl)
    rows.push({ label: 'Remote', value: args.gitUrl, href: args.gitHref })
  if (args.gitCommit)
    rows.push({ label: 'Commit', value: shortCommit(args.gitCommit), href: null })
  return rows
}
